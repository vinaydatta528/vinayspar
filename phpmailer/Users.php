...
class Users
{
	...
	/**
     * Returns all users for Recruiters Management Sastry.
     *
     * @return array users data
     */
    public function getAllRecRec()
    {
		$uidb = $_SESSION['CATS']->getUserID();
        $sql = sprintf(
                "SELECT user.user_id As userID, user.c_by As bdmID,  user.access_level, user.user_name As userName,user.first_name AS firstName,user.last_name As lastName,user2.user_name AS assignedTo FROM USER
LEFT JOIN USER AS user2 ON user2.user_id=user.c_by WHERE user.access_level=200 ORDER BY user.user_id DESC",
                $this->_siteID
                    );
	...
	/**
     * Returns ALL Recruiters of Specific BDM Sastry.
     *
     * @return array users data
     */
    public function getAllR($bdm)
    {
		$uidb = $_SESSION['CATS']->getUserID();
        $sql = sprintf(
                "SELECT
	...
	/**
     * Returns all users for ALL Recruiters Sastry.
     *
     * @return array users data
     */
    public function getAllBDM()
    {
		$uidb = $_SESSION['CATS']->getUserID();
        $sql = sprintf(
                "SELECT
	...
	/**
     * Update Query for Recruiter Mapping Sastry.
     *
     */
public function rtob($blist, $rlist)
    {    
		$sql = sprintf("UPDATE user SET c_by = $blist WHERE user_id = $rlist");
        $this->_db->query($sql);
        return true;
    }
